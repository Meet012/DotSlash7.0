...
with open(r'C:\Users\meetp\OneDrive\Desktop\Coding\Dotslash\Django\crimeRate\DataSet\india_states.geojson', 'r') as file:
                js=json.load(file)
...
with open('result.json', 'r') as fp:
    india_states_geojson = json.load(fp)
...
def crime(request):
    ...
    def plotmap(crime_name):
        "This function plots different crimes on an India map using the state_ut_crimes_df and GeoJson data"
        fig = px.choropleth(
                state_ut_crimes_df,
    ...
    if request.method == 'POST':
        ...
        if state == "":
            ...
            plt_div = plot(plotmap(category))
        else:
            print(state_ut_crimes_df.index)  # Print out the index values
            print(state)
            plt_div = plot(which_state_you_want_to_analyze(state), output_type='div')
    else:
    ...
    return render(request,'plot.html',{"plt_div":plt_div})
